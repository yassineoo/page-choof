...
export default class DigitalAnghamiServices {
  ...
  handlePurchaseClick(e) {
    ...
    const modalContent =
      anghamiModalData[currentLanguage] &&
      anghamiModalData[currentLanguage][offerName];
      ...
      <div class="fixed inset-0 z-[9999] flex items-center justify-center p-4 anghami-modal-fade"
          ...
          <div class="relative bg-white dark:bg-[#2C2C2C] rounded-2xl shadow-2xl w-full max-w-md sm:max-w-lg md:max-w-2xl min-w-[320px] px-6 md:px-8 pt-16 pb-8 md:pb-12" ${dirAttribute}>
              ...
              <div class="text-center mb-6">
                  <h2 id="anghami-modal-title" dir="${!isRTL ? "ltr" : title.includes("1000 OSN+ & ANGHAMI") ? "ltr" : "rtl"}" class="${!isRTL ? "font-rubik" : title.includes("1000 OSN+ & ANGHAMI") ? "font-rubik" : fontClass} font-semibold text-ooredoo-red dark:text-white text-2xl md:text-3xl leading-tight uppercase tracking-tight">
                      ${title}
                  </h2>
              </div>
