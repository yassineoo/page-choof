...
export default class Consommation {
  ...
  setupEventDelegation() {
    this.container.addEventListener('click', (e) => {
      ...
      if (chevronButton) {
        e.preventDefault();
        e.stopPropagation();
        const idx = parseInt(chevronButton.getAttribute('data-card-index'));
  ...
  handleLanguageChange() {
    ...
    if (newLang !== this.state.currentLang || newTheme !== this.state.currentTheme) {
      console.log(`Consommation: Language/Theme changed from ${this.state.currentLang}/${this.state.currentTheme} to ${newLang}/${newTheme}`);
  ...
  async animateMobileCardsReveal() {
    ...
    if (this.state.showAllCards) {
      ...
      const remainingCards = data.cards.slice(1).map(
        (card, index) => this.renderMobileCard(card, index + 1, currentLang, currentTheme, false, this.state.allExpandedPair)
      ).join('');
  ...
  updateButtonText(button, lang, showingAll) {
    ...
    const buttonText = showingAll
      ? (lang === 'ar' ? 'إخفاء التفاصيل' : 'MASQUER DÉTAILS')
...
renderMobileLayout(data, theme, lang) {
    ...
    <div class="w-full flex flex-col ${bgClass} ${fontClass}" ${lang === 'ar' ? 'dir="rtl"' : ''}>
      ...
      <div class="w-full px-4 -mt-7">
        <div class="cards-container flex flex-col gap-6 max-w-full mx-auto">
          ...
          ${this.state.showAllCards ? data.cards.slice(1).map((card, index) => 
  ...
  renderDesktopLayout(data, theme, lang) {
    ...
    return `
      <div class="${bgClass} w-full mb-6 py-8 px-8 md:px-12 flex flex-col items-stretch min-h-screen ${fontClass}" ${lang === 'ar' ? 'dir="rtl"' : ''}>
...
renderSectionSecondaryRow(section, lang, theme) {
  ...
  const hasProgress = !section.infini && !isCreditRecharge &&
    (section.percentage !== undefined && section.percentage !== null);
...
`;
  ...
  renderCardContent(cardIndex) {
    ...
    if (contentElement) {
      ...
      contentElement.innerHTML = sections.map(section =>
