...
export default class Consommation {
  ...
  renderMobileLayout(data, theme, lang) {
    ...
    return `
      <div class="w-full flex flex-col ${bgClass} ${fontClass}" ${
      lang === "ar" ? 'dir="rtl"' : ""
    }>
  ...
  renderSection(section, lang, theme) {
    ...
    if (isFacebookMessenger) {
      const facebookIcon = `<img src="${this.resolveSubIcon(
        section.subIcon[0],
        theme
      )}" style="width:20px;height:20px;" alt="${section.subIcon[0]}" />`;
      const messengerIcon = `<img src="${this.resolveSubIcon(
        section.subIcon[1],
        theme
      )}" style="width:20px;height:20px;" alt="${section.subIcon[1]}" />`;
