const offers = [
  ...
  },
  {
    ...
    features: {
      ...
      ar: ["مكالمات غير محدودة نحو كل الشبكات", "رسائل قصيرة غير محدودة نحو Ooredoo", "50 رسالة قصيرة نحو الشبكات الأخرى"],
    },
  },
  {
    ...
    oldPriceX9: "22500",
    oldPriceX12: "30000",
    duration: { fr: "30 jours", ar: "30 يوم" },
    features: {
      fr: ["Appels illimités vers tous les réseaux", "SMS illimités vers tous les réseaux"],
      ar: ["مكالمات غير محدودة نحو كل الشبكات", "رسائل قصيرة غير محدودة نحو كل الشبكات"],
    },
  },
  {
...
function renderOfferCard(offer, lang, index) {
  ...
  const topLabel = offer.topLabel
    ? `<div class="ooredoo-top-label"><span class="font-noto-kufi-arabic">${offer.topLabel[lang]}</span></div>`
  ...
  const features = offer.features[lang]
    .map((feature) => `
      <div class="ooredoo-feature-item">
        ...
        <span dir="rtl" style="font-size: 20px; font-family: Noto Kufi Arabic">${feature}</span>
      </div>
    `)
    .join("");
    ...
    >
      ...
      <div data-card-view="ooredoo" class="card-view">
        <div class="ooredoo-card-header">
          <h2 class="ooredoo-card-title font-rubik">
        ...
        <div class="ooredoo-card-body ${isArabic ? "is-rtl" : ""}">
          <div class="ooredoo-main-data ${fontClass}">
            <div dir="ltr">
              <span class="ooredoo-main-value font-rubik">${offer.data}</span>
              <span class="ooredoo-main-unit font-rubik">Go</span>
            </div>
            <span class="ooredoo-main-caption">${isArabic ? "إنترنت" : "internet"}</span>
          </div>

          <div dir="ltr" class="ooredoo-feature-list">${features}</div>

          <div class="ooredoo-meta-row">
            <div class="ooredoo-meta-copy">
              <p class="ooredoo-meta-label ${fontClass}">${index === offers.length - 1 ? "استفيدوا من" : isArabic ? "الكل صالح لمدة" : "Valables"}</p>
              <p class="ooredoo-meta-value ${fontClass}">${index === offers.length - 1 ? "11 اشتراك" : offer.duration[lang]}</p>
            </div>
        ...
        <div class="ooredoo-card-body ${isArabic ? "is-rtl" : ""}">
          <div class="ooredoo-forfait-copy ${fontClass}">
            ...
            <p class="ooredoo-forfait-accent">
              <span data-card-free-summary-count style="display: none;">1</span>
          ...
          <div class="ooredoo-forfait-grid">
            ...
            <div class="ooredoo-price-box">
                <span class="ooredoo-old-price font-rubik">
                  <span style="text-decoration: line-through; text-decoration-color: var(--ooredoo-red);" data-card-old-price></span>
                  <span style="text-decoration: none;" class="ooredoo-currency ${fontClass}">
                    ${isArabic ? "دج" : "DA"}
                  </span>
                </span>               <span class="ooredoo-new-price font-rubik">
...
function updateCardMode(card, modeButton) {
  ...
  card.querySelectorAll("[data-card-title]").forEach((el) => {
    el.textContent =
      nextMode === "forfait"
        ? `${selectedPack === "X6" ? "6X" : selectedPack === "X9" ? "9X" : "12X"} ${baseName}`
        : baseName;
  });
...
function initSliderDots() {
    ...
    slider.scrollTo({
        left: isRTL
            ? slider.scrollWidth - slider.clientWidth * (index + 1)
