...
export class Slider {
  ...
  createForfaitCard(offer, index, labels) {
    ...
    const buyLabel = (isRTL ? "تغيير" : "Changer");
    ...
    return `
        <div>
          <div class="hidden sm:flex w-full items-center justify-center">
            <div 
              class="grid items-stretch grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 justify-center gap-5 lg:gap-2 max-w-[1300px] grid-center-last-2"
            > 
          ...
          <div class="w-full mt-10 px-4 lg:px-8 max-w-[1300px] mx-auto text-[20px]">
              <p>${this.currentLang === "ar" ? "(1) إنترنت بتدفق منخفض بعد نفاذ اشتراك الإنترنت." : "(1) Débit réduit après épuisement du volume internet."}</p>
              <p>${this.currentLang === "ar" ? "(2) منها 20Go قابلة للاستعمال في تونس." : "(2) Dont 20Go utilisables en Tunisie."}</p>
            </div>
        </div>`;
  }
  ...
  createResponsiveLayoutInternet(offers, labels, gridType, isRTL, convertToLatinNumerals) {
    ...
    return `
          <div class="hidden sm:flex w-full items-center justify-center">
            <div class="gap-5 items-stretch grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 px-2">
              ${offers
