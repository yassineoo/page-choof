...
class ForfaitComponent {
  constructor(container) {
    ...
    this.sliders = new Map([["forfaits", this.createSliderState()]]);
    this.boundHandlers = {
...
@media (min-width: 768px) and (max-width: 991px) {
  ...
  `;
    ...
    this.bindPurchaseButtons(language, [...data.forfaits]);
  ...
  // Add this method to fix Arabic number display
  fixArabicNumbers(text) {
    // Find numbers with spaces (like "50 000" or "30 000")
    return text.replace(/(\d+)\s+(\d+)/g, (match, first, second) => {
      // Wrap the complete number in a span with LTR direction
      return `<span dir="ltr">${first} ${second}</span>`;
    });
  }

  // Update your createModalHTML method - fix the title line:
  createModalHTML({ type, title, message, isRTL }) {
    ...
    // Fix Arabic numbers in the title
    const fixedTitle = isRTL ? this.fixArabicNumbers(title) : title;
