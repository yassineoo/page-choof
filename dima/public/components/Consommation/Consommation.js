...
export default class Consommation {
  ...
  handleLanguageChange() {
    ...
    if (newLang !== this.state.currentLang || newTheme !== this.state.currentTheme) {
      console.log(
        `Consommation: Language/Theme changed from ${this.state.currentLang}/${this.state.currentTheme} to ${newLang}/${newTheme}`,
      )
  ...
  resolveIcon(name, theme) {
    ...
    if (n === "messenger") {
      ...
      iconPath =
        theme === "dark" ? `${this.config.IMAGE_BASE}dark/${n}-white.svg` : `${this.config.IMAGE_BASE}light/${n}.svg`
    } else {
  ...
  async animateMobileCardsReveal() {
    ...
    if (this.state.showAllCards) {
      ...
      const remainingCards = data.cards
        .slice(1)
        .map((card, index) =>
          this.renderMobileCard(card, index + 1, currentLang, currentTheme, false, this.state.allExpandedPair),
        )
        .join("")
  ...
  renderSectionIcons(section, theme) {
    ...
    if (Array.isArray(section.subIcon)) {
      return section.subIcon
        .map(
          (icon) => `<img src="${this.resolveSubIcon(icon, theme)}" style="width:20px;height:20px;" alt="${icon}" />`,
        )
        .join("")
    }
  ...
  renderSectionSecondaryRow(section, lang, theme) {
    ...
    const hasProgress =
      !section.infini && !isCreditRecharge && section.percentage !== undefined && section.percentage !== null
  ...
  renderCardContent(cardIndex) {
    ...
    if (contentElement) {
      ...
      contentElement.innerHTML = sections
        .map((section) => this.renderSection(section, currentLang, currentTheme))
        .join("")
    }
  }
